...
```bash
# Check config
cat ~/.llamafarm/config.toml | grep -A 5 telegram
...
| Health check | <5s | `time llamafarm channel doctor` |
...
5. **Monitor logs** for any issues:
   ...
   llamafarm daemon
   # or
   RUST_LOG=info llamafarm channel start
   ```
...
- Issues: https://github.com/theonlyhennygod/llamafarm/issues
