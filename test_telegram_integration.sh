#!/bin/bash
# LlamaFarm Telegram Integration Test Suite
...
if [ -f "target/release/llamafarm" ]; then
    BINARY_SIZE=$(ls -lh target/release/llamafarm | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/llamafarm 2>/dev/null || stat -c%s target/release/llamafarm)
...
CONFIG_PATH="$HOME/.llamafarm/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'llamafarm onboard' first"
    fi
else
    warn "No config file found - run 'llamafarm onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/llamafarm channel doctor 2>&1 || true)
END_TIME=$(date +%s)
