...
Examples:
  ...
  llamafarm channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  llamafarm channel bind-telegram 123456789")]
    BindTelegram {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `LlamaFarm` workspace
    Openclaw {
...
Examples:
  ...
  llamafarm cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  llamafarm cron add-every 60000 'Ping heartbeat'     # every minute
  llamafarm cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  llamafarm cron once 2h 'Follow up on deployment'
  llamafarm cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  llamafarm cron update <task-id> --expression '0 8 * * *'
  llamafarm cron update <task-id> --tz Europe/London --name 'Morning check'
  llamafarm cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  llamafarm hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  llamafarm peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash LlamaFarm firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash LlamaFarm firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  llamafarm peripheral flash
  llamafarm peripheral flash --port /dev/cu.usbmodem12345
  llamafarm peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash LlamaFarm firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
