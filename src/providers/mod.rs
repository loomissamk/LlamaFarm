...
pub struct ProviderRuntimeOptions {
    ...
    pub llamafarm_dir: Option<PathBuf>,
...
impl ProviderRuntimeOptions {
    /// Build the provider controls shared by CLI, channels, gateway, and
    /// delegated agents. Keeping this mapping in one place prevents a saved
    /// provider setting from applying to only one application surface.
    #[must_use]
    pub fn from_config(config: &crate::config::Config) -> Self {
        Self {
            auth_profile_override: None,
            provider_api_url: config.api_url.clone(),
            llamafarm_dir: config.config_path.parent().map(PathBuf::from),
...
/// 3. Generic fallback variables (`LLAMAFARM_API_KEY`, `API_KEY`)
///
...
fn resolve_provider_credential(name: &str, credential_override: Option<&str>) -> Option<String> {
    ...
    for env_var in ["LLAMAFARM_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
fn create_provider_with_url_and_options(
    ...
    match name {
        ...
        "gemini" | "google" | "google-gemini" => {
            let state_dir = options.llamafarm_dir.clone().unwrap_or_else(|| {
                directories::UserDirs::new().map_or_else(
                    || PathBuf::from(".llamafarm"),
                    |dirs| dirs.home_dir().join(".llamafarm"),
                )
            });
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `llamafarm onboard --interactive` to reconfigure.\n\
...
/// Return the list of all known providers for display in `llamafarm providers list`.
///
...
mod tests {
    ...
    fn resolve_qwen_oauth_context_prefers_explicit_override() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/llamafarm-qwen-oauth-home-{}", std::process::id());
    ...
    fn resolve_qwen_oauth_context_uses_env_token_and_resource_url() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/llamafarm-qwen-oauth-home-{}-env", std::process::id());
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, Some("oauth-token"));
    ...
    fn resolve_qwen_oauth_context_reads_cached_credentials_file() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/llamafarm-qwen-oauth-home-{}-file", std::process::id());
    ...
    fn resolve_qwen_oauth_context_placeholder_does_not_use_dashscope_fallback() {
        ...
        let fake_home = format!(
            "/tmp/llamafarm-qwen-oauth-home-{}-placeholder",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, None);
    ...
    fn provider_credential_available_qwen_oauth_accepts_refresh_token_without_live_refresh() {
        let _env_lock = env_lock();
        let fake_home = format!(
            "/tmp/llamafarm-qwen-oauth-home-{}-available-refresh",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, None);
    ...
    fn provider_credential_available_qwen_oauth_rejects_placeholder_without_sources() {
        let _env_lock = env_lock();
        let fake_home = format!(
            "/tmp/llamafarm-qwen-oauth-home-{}-available-none",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, None);
