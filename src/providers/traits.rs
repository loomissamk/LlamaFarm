...
pub trait Provider: Send + Sync {
    ...
    /// Per-model native tool support from cached provider metadata (e.g. Ollama `/api/show`).
    /// Returns `Some(true/false)` when the provider has cached capability data for this model,
    /// or `None` when unknown (falls back to `supports_native_tools` + config).
    fn cached_model_tool_support(&self, _model: &str) -> Option<bool> {
        None
    }
    ...
    /// Prefetch and cache per-model capabilities (e.g. tool support) so that
    /// `cached_model_tool_support` returns a definitive answer before the first
