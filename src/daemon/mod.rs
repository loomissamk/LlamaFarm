...
pub async fn run(config: Config, host: String, port: u16) -> Result<()> {
    ...
    let reaped = crate::agent::run_ledger::reap_stale_running_runs(&config.workspace_dir);
    if reaped > 0 {
        tracing::info!(
            reaped,
            "marked stale 'running' run ledgers as cancelled after restart"
        );
    }
    ...
    println!("🧠 LlamaFarm daemon started");
