//! Flash LlamaFarm Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash LlamaFarm Nucleo firmware. Builds from firmware/llamafarm-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (llamafarm's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("llamafarm-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from llamafarm repo root.",
            firmware_dir.display()
        );
    }

    println!("Building LlamaFarm Nucleo firmware...");
    ...
    println!("LlamaFarm Nucleo firmware flashed successfully.");
