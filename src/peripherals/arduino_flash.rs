//! Flash LlamaFarm Arduino firmware via arduino-cli.
//!
...
/// LlamaFarm Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/llamafarm-arduino/llamafarm-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "llamafarm-arduino";
...
/// Flash LlamaFarm firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("llamafarm_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling LlamaFarm Arduino firmware...");
    ...
    println!("LlamaFarm firmware flashed successfully.");
