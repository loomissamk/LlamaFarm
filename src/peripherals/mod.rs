...
/// Handle `llamafarm peripheral` subcommands.
    ...
    match cmd {
        crate::PeripheralCommands::List => {
            ...
            if boards.is_empty() {
                ...
                println!();
                println!("Add one with: llamafarm peripheral add <board> <path>");
                println!("  Example: llamafarm peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
