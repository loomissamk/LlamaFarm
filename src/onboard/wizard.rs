use crate::config::schema::{
    ...
    QQConfig, QQReceiveMode, SignalConfig, StreamMode, WhatsAppConfig, default_nostr_relays,
};
use crate::config::{
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to LlamaFarm — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 10, "Channels (How You Talk to LlamaFarm)");
    ...
    let config = Config {
        ...
        host_runner: crate::config::HostRunnerConfig::default(),
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("LLAMAFARM_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to LlamaFarm)");
...
/// Use `llamafarm onboard` or `llamafarm onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `llamafarm onboard --interactive` for the full wizard.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".llamafarm");
...
async fn run_quick_setup_with_home(
    ...
    let (llamafarm_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     llamafarm agent -m \"Hello!\"");
            println!("    2. Gateway:  llamafarm gateway");
            println!("    3. Status:   llamafarm status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             llamafarm auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              llamafarm agent -m \"Hello!\"");
                println!("    3. Gateway:           llamafarm gateway");
                println!("    4. Status:            llamafarm status");
            }
        } else {
            let env_var = provider_env_var(&provider_name);
            println!("    1. Set your API key:  export {env_var}=\"sk-...\"");
            println!("    2. Or edit:           ~/.llamafarm/config.toml");
            println!("    3. Chat:              llamafarm agent -m \"Hello!\"");
            println!("    4. Gateway:           llamafarm gateway");
        }
    } else {
        println!("    1. Chat:     llamafarm agent -m \"Hello!\"");
        println!("    2. Gateway:  llamafarm gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `llamafarm models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: llamafarm models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
const LLAMAFARM_ONBOARD_NONINTERACTIVE: &str = "LLAMAFARM_ONBOARD_NONINTERACTIVE";

fn onboard_should_force_noninteractive() -> bool {
    std::env::var(LLAMAFARM_ONBOARD_NONINTERACTIVE).is_ok()
}
...
fn ensure_onboard_overwrite_allowed(config_path: &Path, force: bool) -> Result<()> {
    ...
    if onboard_should_force_noninteractive()
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet(
            "LlamaFarm works with ANY API that speaks the OpenAI chat completions format.",
        );
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("LlamaFarm will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how LlamaFarm connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("LlamaFarm uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("LlamaFarm can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("LlamaFarm can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should LlamaFarm interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how LlamaFarm stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to LlamaFarm from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to LlamaFarm from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to LlamaFarm from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to LlamaFarm from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("LlamaFarm reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.llamafarm/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("llamafarm-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via LLAMAFARM_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::LarkFeishu => {
                ...
                println!(
                    ...
                    style("— talk to LlamaFarm from Lark or Feishu").dim()
                );
                print_bullet(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("LlamaFarm will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("LlamaFarm is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("llamafarm auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: llamafarm auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("llamafarm channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("llamafarm agent -m \"Hello, LlamaFarm!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("llamafarm agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("llamafarm status").yellow());
...
mod tests {
    ...
    async fn run_quick_setup_with_clean_env(
        ...
        let _workspace_env = EnvVarGuard::unset("LLAMAFARM_WORKSPACE");
        let _config_env = EnvVarGuard::unset("LLAMAFARM_CONFIG_DIR");
        let _noninteractive_env = EnvVarGuard::set(LLAMAFARM_ONBOARD_NONINTERACTIVE, "1");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        let tmp = TempDir::new().unwrap();
        let llamafarm_dir = tmp.path().join(".llamafarm");
        let config_path = llamafarm_dir.join("config.toml");

        tokio::fs::create_dir_all(&llamafarm_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("llamafarm-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".llamafarm").join("config.toml");
        ...
        .await
        .expect("quick setup should honor LLAMAFARM_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** LlamaFarm"),
            "should default agent name to LlamaFarm"
        );
