...
fn normalize_shell_command_from_array(items: &[serde_json::Value]) -> Option<String> {
...
fn normalize_task_plan_arguments(
    ...
    fn normalize_task_plan_action(raw: &str) -> Option<&'static str> {
        match raw.trim().to_ascii_lowercase().as_str() {
            "create" | "new" | "init" | "plan" => Some("create"),
            "add" | "append" => Some("add"),
            "update" | "set" | "mark" => Some("update"),
            "list" | "read" | "get" | "show" | "ls" => Some("list"),
            "delete" | "clear" | "reset" | "remove" => Some("delete"),
            _ => None,
        }
    }
    ...
    match arguments {
        serde_json::Value::Object(mut map) => {
            if let Some(update) = extract_update_from_items(&map) {
                ...
                map.remove("steps");
                map.remove("updates");
            } else if let Some(tasks) = map.get("tasks").and_then(serde_json::Value::as_array) {
            ...
            if map
                .get("action")
                .and_then(serde_json::Value::as_str)
                .and_then(normalize_task_plan_action)
                .is_none()
            {
                if let Some(action) =
                    extract_alias_string(&map, &["hint", "operation", "op", "mode"])
                        .and_then(|value| normalize_task_plan_action(&value))
                {
                    map.insert(
                        "action".to_string(),
                ...
                } else if let Some(raw) = raw_string_hint.and_then(normalize_task_plan_action) {
                    map.insert(
                        "action".to_string(),
        ...
        serde_json::Value::String(raw) => normalize_task_plan_action(&raw)
...
pub(super) fn normalize_shell_arguments(
    ...
    fn command_from_shell_json(value: &serde_json::Value) -> Option<String> {
        match value {
            serde_json::Value::String(raw) => {
                if let Some(command) = normalize_shell_command_from_raw(raw) {
                    return Some(command);
                }

                let parsed = serde_json::from_str::<serde_json::Value>(raw.trim()).ok()?;
                command_from_shell_json(&parsed)
            }
            serde_json::Value::Array(items) => normalize_shell_command_from_array(items),
            serde_json::Value::Object(map) => {
                if let Some(command) = map.get("argv").and_then(|value| match value {
                    serde_json::Value::Array(items) => normalize_shell_command_from_array(items),
                    _ => None,
                }) {
                    return Some(command);
    ...
    match arguments {
        serde_json::Value::Object(mut map) => {
            if let Some(command) = map.get("command").and_then(command_from_shell_json) {
            ...
            for alias in [
                ...
                if let Some(command) = map.get(alias).and_then(command_from_shell_json) {
                    map.insert("command".to_string(), serde_json::Value::String(command));
                    return serde_json::Value::Object(map);
                }
            }
...
fn canonicalize_web_search_signature(arguments: &serde_json::Value) -> serde_json::Value {
    let mut canonical = canonicalize_json_for_tool_signature(arguments);
    if let Some(obj) = canonical.as_object_mut() {
        if let Some(hint) = obj.get("hint").and_then(|v| v.as_str()) {
            let normalized = hint.trim().to_ascii_lowercase();
            obj.insert("hint".to_string(), serde_json::Value::String(normalized));
        }
        if let Some(query) = obj.get("query").and_then(|v| v.as_str()) {
            let normalized = query.trim().to_ascii_lowercase();
            obj.insert("query".to_string(), serde_json::Value::String(normalized));
        }
    }
    canonical
}
...
pub(super) fn tool_call_signature(name: &str, arguments: &serde_json::Value) -> (String, String) {
    ...
    let canonical_args = match normalized_name.as_str() {
        ...
        "web_search_tool" => canonicalize_web_search_signature(arguments),
        ...
        last_consumed = match boundary_kind {
            ...
            XmlishTagBoundary::OpenTagClosed => {
                ...
                if let Some(close_rel) = response[inner_start..].find(&closing_tag) {
                    ...
                    // No closing tag (unclosed attribute-style tag); consumed up
                    // to and including the '>' of the open tag.
                    inner_start
                }
            }
        };
...
fn parse_trailing_standalone_json_tool_calls(
    ...
    for line_start in candidate_starts.into_iter().rev() {
        ...
        let raw_before = response[..json_start].trim_end();
        let before = trim_trailing_meta_close_tags(raw_before).trim();

        // Security: plain prose followed by raw JSON is a prompt-injection risk
        // (the text could have been injected, not generated by the model).
        // Only accept trailing JSON when the before-text is empty (pure JSON
        // response) or ends with a reasoning close tag like </think>, which
        // signals a model-native thinking-then-acting pattern.
        let had_reasoning_tag = raw_before != trim_trailing_meta_close_tags(raw_before);
        if !before.is_empty() && !had_reasoning_tag {
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn normalize_shell_arguments_recovers_command_from_argv_alias() {
        assert_eq!(
            normalize_tool_arguments("shell", json!({ "cmd": ["bash", "-lc", "ls -R"] }), None),
            json!({
                "cmd": ["bash", "-lc", "ls -R"],
    ...
    #[test]
    fn normalize_shell_arguments_recovers_command_from_top_level_argv_array() {
        assert_eq!(
            normalize_tool_arguments("shell", json!(["bash", "-lc", "mkdir rust_kernel"]), None),
            json!({
                "command": "bash -lc 'mkdir rust_kernel'"
            })
        );
    }

    #[test]
    fn parse_tool_calls_recovers_shell_command_from_root_cmd_array_json() {
        let response = r#"{"cmd":["bash","-lc","mkdir rust_kernel"]}"#;
    ...
    #[test]
    fn normalize_task_plan_arguments_recovers_update_from_tasks_step_status() {
        assert_eq!(
            normalize_tool_arguments(
                "task_plan",
                json!({ "action": "update", "tasks": [{ "step": 1, "status": "completed" }] }),
                None,
            ),
            json!({
                "action": "update",
    ...
    #[test]
    fn normalize_task_plan_arguments_maps_read_alias_to_list() {
        assert_eq!(
            normalize_tool_arguments("task_plan", json!({ "action": "read" }), None),
            json!({
                "action": "list"
            })
        );
