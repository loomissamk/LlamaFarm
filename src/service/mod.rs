...
const SERVICE_LABEL: &str = "com.llamafarm.daemon";
const WINDOWS_TASK_NAME: &str = "LlamaFarm Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "llamafarm.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["llamafarm", "start"]))?;
        }
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["llamafarm", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/llamafarm");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/llamafarm");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "llamafarm", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove llamafarm from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/llamafarm)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: llamafarm service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "llamafarm.service"]));
...
/// Check if the llamafarm user exists and has expected properties.
...
fn check_llamafarm_user() -> Result<()> {
    ...
    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser llamafarm && delgroup llamafarm",
            "addgroup -S llamafarm && adduser -S -s /sbin/nologin -H -D -G llamafarm llamafarm",
        )
    } else {
        ("userdel llamafarm", "useradd -r -s /sbin/nologin llamafarm")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'llamafarm' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/llamafarm" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: llamafarm user has home directory '{}' (expected /var/lib/llamafarm or /nonexistent)",
                        home
                    );
                }
...
fn ensure_llamafarm_user() -> Result<()> {
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "llamafarm"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "llamafarm"])
                .output()
                .context("Failed to create llamafarm group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create llamafarm group: {}", stderr.trim());
            }
            println!("✅ Created system group: llamafarm");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create llamafarm user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create llamafarm user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "llamafarm"])
            .output()
            .context("Failed to create llamafarm user")?;
    ...
    println!("✅ Created system user: llamafarm");
    Ok(())
}

/// Change ownership of a path to llamafarm:llamafarm
#[cfg(unix)]
fn chown_to_llamafarm(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["llamafarm:llamafarm", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to llamafarm:llamafarm: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_llamafarm(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_llamafarm(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "llamafarm:llamafarm", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to llamafarm:llamafarm: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_llamafarm(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".llamafarm"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'llamafarm' cannot write {} ({details}). \
             Re-run `sudo llamafarm service install` and ensure ownership is llamafarm:llamafarm.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/llamafarm",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="llamafarm:llamafarm"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/llamafarm/access.log"
error_log="/var/log/llamafarm/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/llamafarm");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo llamafarm service install"
        );
    }

    ensure_llamafarm_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/llamafarm");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/llamafarm");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_llamafarm(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by llamafarm:llamafarm)",
    ...
    chown_recursive_to_llamafarm(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/llamafarm");
    ...
    run_checked(Command::new("rc-update").args(["add", "llamafarm", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/llamafarm");
    println!("   Config path: /etc/llamafarm/config.toml");
    println!("   Start with: sudo llamafarm service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("llamafarm-daemon.cmd");
...
mod tests {
    ...
    fn run_capture_falls_back_to_stderr() {
        let out = run_capture(Command::new("env").args(["-i", "sh", "-lc", "echo warn 1>&2"]))
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/llamafarm.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "LlamaFarm Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/llamafarm");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/llamafarm"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"llamafarm\""));
        assert!(script.contains("description=\"LlamaFarm daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/llamafarm\""));
        assert!(script.contains("command_args=\"--config-dir /etc/llamafarm daemon\""));
        assert!(!script.contains("env LLAMAFARM_CONFIG_DIR"));
        assert!(!script.contains("env LLAMAFARM_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"llamafarm:llamafarm\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/llamafarm/access.log\""));
        assert!(script.contains("error_log=\"/var/log/llamafarm/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/llamafarm");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/llamafarm");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/llamafarm");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/llamafarm"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/llamafarm'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/llamafarm/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/llamafarm/workspace'".to_string(),
                "llamafarm".to_string()
            ]
        );
    }
