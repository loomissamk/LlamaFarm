...
impl Tool for AgentsSendTool {
    ...
    fn description(&self) -> &str {
        "Send a coordination message to a specific agent by ID, or broadcast to all agents with to_agent=\"*\". \
         Use this ONLY for inter-agent coordination signals (e.g. notifying peers of a result, sharing state). \
         Do NOT use this to delegate tasks you can execute directly — use shell_exec, file_write, or other available tools instead."
    }
