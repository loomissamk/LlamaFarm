...
impl ProcessTool {
    ...
    fn handle_spawn(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let working_dir_allowed = if execution_plan.cwd.is_absolute() {
            let resolved = execution_plan
                .cwd
                .canonicalize()
                .unwrap_or_else(|_| execution_plan.cwd.clone());
            self.security.is_resolved_path_allowed(&resolved)
        } else {
            self.security
                .is_path_allowed(&execution_plan.cwd.to_string_lossy())
        };
