...
pub fn all_tools_with_runtime_and_federation(
    ...
    tool_arcs.push(Arc::new(
        SopStatusTool::new(sop_engine.clone()).with_collector(sop_metrics.clone()),
    ));
    tool_arcs.push(Arc::new(
    ...
    tool_arcs.push(Arc::new(
        SopApproveTool::new(sop_engine.clone())
            ...
            .with_collector(sop_metrics.clone()),
    ));
    tool_arcs.push(Arc::new(
    ...
    // arXiv semantic search (requires Qdrant populated via arxiv_to_qdrant.py)
    if root_config.arxiv_rag.enabled {
        tool_arcs.push(Arc::new(ArxivSearchTool::new(
            &root_config.arxiv_rag.qdrant_url,
            &root_config.arxiv_rag.embed_url,
            &root_config.arxiv_rag.embed_model,
            &root_config.arxiv_rag.collection,
        )));
    }

    // Generic database explorer tools (db_schema + db_query)
    if !root_config.db_connections.is_empty() {
        let conns = root_config.db_connections.clone();
        tool_arcs.push(Arc::new(DbSchemaTool::new(conns.clone())));
        tool_arcs.push(Arc::new(DbQueryTool::new(conns)));
    }
    ...
    // Add delegation and sub-agent orchestration tools when agents are configured and delegation is enabled
    if root_config.federation.enable_delegation && (!agents.is_empty() || federation.is_some()) {
        ...
        let provider_runtime_options =
            crate::providers::ProviderRuntimeOptions::from_config(root_config);
        ...
        .with_max_history_messages(root_config.agent.max_history_messages);
