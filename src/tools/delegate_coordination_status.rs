...
impl Tool for DelegateCoordinationStatusTool {
    ...
    fn description(&self) -> &str {
        "Inspect delegate coordination runtime state (agent inbox backlog, context state transitions, and dead-letter events). Only call this when you have already dispatched a delegate task and are waiting for its result. Do NOT call this to search for available workers or to check if peers exist — if no delegate contexts are active, proceed with direct execution using your available tools."
    }
