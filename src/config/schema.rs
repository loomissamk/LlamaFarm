...
pub struct Config {
    ...
    /// arXiv semantic search configuration (`[arxiv_rag]`).
    #[serde(default)]
    pub arxiv_rag: ArxivRagConfig,

    /// Generic database connections for the explorer tab and db_query/db_schema tools.
    /// Each entry is a named connection reachable from the DB Explorer UI and LLM tools.
    ///
    /// Example:
    /// ```toml
    /// [[db_connections]]
...
/// arXiv semantic search RAG configuration (`[arxiv_rag]` section).
///
/// Enables the `arxiv_search` tool which queries a Qdrant collection pre-populated
/// by the `arxiv_to_qdrant.py` ingest script against ArXivDB.Papers on the NAS.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct ArxivRagConfig {
    /// Enable the arxiv_search tool. Requires Qdrant populated with paper embeddings.
    #[serde(default)]
    pub enabled: bool,
    /// Qdrant server URL (Docker service name resolves within the bundle network).
    #[serde(default = "default_arxiv_qdrant_url")]
    pub qdrant_url: String,
    /// Ollama base URL for generating query embeddings via /v1/embeddings.
    #[serde(default = "default_arxiv_embed_url")]
    pub embed_url: String,
    /// Embedding model name — must match the model used during ingest.
    #[serde(default = "default_arxiv_embed_model")]
    pub embed_model: String,
    /// Qdrant collection name for the arXiv paper index.
    #[serde(default = "default_arxiv_collection")]
    pub collection: String,
}

fn default_arxiv_qdrant_url() -> String {
    "http://qdrant:6333".to_string()
}
fn default_arxiv_embed_url() -> String {
    "http://127.0.0.1:11434".to_string()
}
fn default_arxiv_embed_model() -> String {
    "nomic-embed-text".to_string()
}
fn default_arxiv_collection() -> String {
    "arxiv_papers".to_string()
}

impl Default for ArxivRagConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            qdrant_url: default_arxiv_qdrant_url(),
            embed_url: default_arxiv_embed_url(),
            embed_model: default_arxiv_embed_model(),
            collection: default_arxiv_collection(),
        }
    }
}

/// Database driver type for the generic DB explorer.
...
/// A named database connection for the DB Explorer tab and `db_query`/`db_schema` LLM tools.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct DbConnectionConfig {
    /// Unique name used to reference this connection (e.g. "arxiv", "local_sqlite").
    pub name: String,
    /// Database driver: sqlite, postgres, mysql, or mongodb.
    pub driver: DbDriver,
    ...
    /// Restrict to read-only queries (SELECT/EXPLAIN for SQL, find for MongoDB).
    #[serde(default = "default_true")]
    pub read_only: bool,
    /// Maximum number of rows returned per query. Default: 500.
    #[serde(default = "default_db_max_rows")]
    pub max_rows: usize,
    /// Optional human-readable label shown in the UI. Falls back to `name`.
    #[serde(default)]
...
pub struct GatewayConfig {
    ...
    pub host: String,
    /// Legacy compatibility field. Gateway pairing is retired and ignored.
    #[serde(default)]
...
pub fn build_runtime_proxy_client_with_timeouts(
    ...
    build_runtime_proxy_client_with_optional_timeouts(
        service_key,
        Some(timeout_secs),
...
/// Build a proxy-aware HTTP client with explicitly optional total and connect
/// deadlines. Passing `None` for the total timeout leaves the builder without
/// a response deadline. Long-running local inference uses this with a bounded
/// connect timeout and no arbitrary generation deadline; it remains
/// cancellable by dropping the request future.
pub fn build_runtime_proxy_client_with_optional_timeouts(
    service_key: &str,
    timeout_secs: Option<u64>,
    connect_timeout_secs: Option<u64>,
) -> reqwest::Client {
    let cache_key = runtime_proxy_cache_key(service_key, timeout_secs, connect_timeout_secs);
    if let Some(client) = runtime_proxy_cached_client(&cache_key) {
        return client;
    }
...
impl AgentExecutionMode {
    ...
    pub fn is_autonomous(&self) -> bool {
        matches!(
            self,
            Self::Operator | Self::AutonomousOperator | Self::ChaosLab
        )
    }
}
...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            ...
            arxiv_rag: ArxivRagConfig::default(),
...
impl Config {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        // Agent execution. A zero token budget would make every provider
    ...
    pub fn apply_env_overrides(&mut self) {
        ...
        if let Ok(mode) = std::env::var("LLAMAFARM_DISCOVERY_MODE") {
            match mode.trim().to_ascii_lowercase().as_str() {
                ...
                _ => {
                    tracing::warn!("Ignoring invalid LLAMAFARM_DISCOVERY_MODE (valid: mdns|manual)")
                }
            }
        }
        ...
        // Agent runtime controls: keep explicit deployment profiles in sync
        // with the tool loop rather than relying on hard-coded defaults.
        if let Ok(enabled) = std::env::var("LLAMAFARM_AGENT_COMPACT_CONTEXT") {
            match enabled.trim().to_ascii_lowercase().as_str() {
                "1" | "true" | "yes" | "on" => self.agent.compact_context = true,
                "0" | "false" | "no" | "off" => self.agent.compact_context = false,
                _ => tracing::warn!(
                    "Ignoring invalid LLAMAFARM_AGENT_COMPACT_CONTEXT (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }
        ...
        if let Ok(enabled) = std::env::var("LLAMAFARM_AGENT_PARALLEL_TOOLS") {
            match enabled.trim().to_ascii_lowercase().as_str() {
                "1" | "true" | "yes" | "on" => self.agent.parallel_tools = true,
...
"#;
    ...
    #[test]
    async fn checklist_gateway_default_disables_pairing() {
        let g = GatewayConfig::default();
        assert!(!g.require_pairing, "Gateway pairing must stay disabled");
    }
    ...
    async fn checklist_gateway_cli_default_host_is_localhost() {
        ...
        assert!(
            ...
            "Config default must disable pairing"
        );
        assert!(
...
"#,
    ...
    #[test]
    async fn env_override_require_pairing_is_ignored() {
        let _env_guard = env_override_lock().await;
        let mut config = Config::default();
        assert!(!config.gateway.require_pairing);

        std::env::set_var("LLAMAFARM_REQUIRE_PAIRING", "true");
        config.apply_env_overrides();
        assert!(!config.gateway.require_pairing);
    ...
    #[test]
    async fn env_override_agent_runtime_controls() {
        let _env_guard = env_override_lock().await;
        let mut config = Config::default();
