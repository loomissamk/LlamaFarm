...
pub async fn handle_static(uri: Uri) -> impl IntoResponse {
    ...
    // SPA fallback: extensionless paths (/_app/workspace, /_app/agent, ...)
    // are client-side routes, not files — serve index.html so hard refreshes
    // and direct links work instead of 404ing.
    if WebAssets::get(path).is_none() && !path.rsplit('/').next().unwrap_or("").contains('.') {
