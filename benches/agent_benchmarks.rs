//! Performance benchmarks for LlamaFarm hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/llamafarm-labs/llamafarm/issues/618 (item 7)
...
use llamafarm::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use llamafarm::config::MemoryConfig;
use llamafarm::memory;
use llamafarm::memory::{Memory, MemoryCategory};
...
<tool_call>
{"name": "search", "arguments": {"query": "llamafarm architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "llamafarm"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about llamafarm agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("llamafarm agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
