# LlamaFarm on Nucleo-F401RE — Step-by-Step Guide

Run LlamaFarm on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

LlamaFarm can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
LlamaFarm includes everything for Nucleo-F401RE:
...
| Flash command | `llamafarm peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
This builds `firmware/llamafarm-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `llamafarm daemon` or `llamafarm agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `llamafarm peripheral list` to verify.
