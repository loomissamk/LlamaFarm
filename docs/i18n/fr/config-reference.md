...
- `agent.max_output_tokens_per_turn` vaut `16384` par défaut et limite uniquement un segment de raisonnement/sortie ; la même tâche reprend depuis un checkpoint lorsque cette limite est atteinte.
- `agent.max_no_progress_spins` vaut `6` par défaut : seuls des segments de raisonnement vides ou des appels tool identiques consécutifs déclenchent l'arrêt, et tout progrès réel réinitialise immédiatement le compteur.
...
- `provider.ollama_num_ctx` est une valeur manuelle exacte ; le tableau de bord
  accepte 2 048–262 144. Sans cette valeur, Auto utilise le maximum natif du modèle.
- Avec `LLAMAFARM_ADAPTIVE_CONTEXT=true`, `OLLAMA_NUM_CTX` devient la
