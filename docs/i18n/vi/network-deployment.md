# Triển khai mạng — LlamaFarm trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai LlamaFarm trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | LlamaFarm poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | LlamaFarm sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — LlamaFarm thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. LlamaFarm trên Raspberry Pi
...
Chỉnh sửa `~/.llamafarm/config.toml`:
...
- Channel Telegram hoạt động: LlamaFarm poll Telegram API (outbound)
...
```bash
llamafarm daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `llamafarm tunnel` (xem tài liệu tunnel).

3. LlamaFarm sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- LlamaFarm gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `llamafarm daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `llamafarm daemon`).
...
- [ ] Chạy `llamafarm daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
