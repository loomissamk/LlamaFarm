...
- `cargo check` / `cargo build` dừng lâu ở `Checking llamafarm`
...
### Không tìm thấy lệnh `llamafarm` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `llamafarm`
...
Xác minh `~/.llamafarm/config.toml`:
...
- Dừng các tiến trình `llamafarm daemon` / `llamafarm channel start` thừa
