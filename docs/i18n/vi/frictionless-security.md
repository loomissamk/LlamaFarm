...
[INFO] ✓ Audit logging enabled (~/.config/llamafarm/audit.log)
...
$ llamafarm security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/llamafarm/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ llamafarm config set security.sandbox.enabled false

# Bật backend cụ thể
$ llamafarm config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ llamafarm config set security.resources.max_memory_mb 2048
```
...
```bash
$ llamafarm onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `llamafarm security --status`
