# Thêm Board và Tool — Hướng dẫn phần cứng LlamaFarm

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào LlamaFarm.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.llamafarm/config.toml)
llamafarm peripheral add nucleo-f401re /dev/ttyACM0
llamafarm peripheral add arduino-uno /dev/cu.usbmodem12345
llamafarm peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, LlamaFarm có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `llamafarm peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `llamafarm peripheral list` | Liệt kê các board đã cấu hình |
| `llamafarm peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `llamafarm peripheral flash` | Nạp firmware Arduino |
| `llamafarm peripheral flash-nucleo` | Nạp firmware Nucleo |
| `llamafarm hardware discover` | Liệt kê thiết bị USB |
| `llamafarm hardware info` | Thông tin chip qua probe-rs |
