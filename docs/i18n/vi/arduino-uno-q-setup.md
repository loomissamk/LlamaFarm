# LlamaFarm trên Arduino Uno Q — Hướng dẫn từng bước

Chạy LlamaFarm trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

LlamaFarm bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**

| Thành phần | Vị trí | Mục đích |
|------------|--------|---------|
| Bridge app | `firmware/llamafarm-uno-q-bridge/` | MCU sketch + Python socket server (port 9999) cho GPIO |
| Bridge tools | `src/peripherals/uno_q_bridge.rs` | Tool `gpio_read` / `gpio_write` giao tiếp với Bridge qua TCP |
| Setup command | `src/peripherals/uno_q_setup.rs` | `llamafarm peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt LlamaFarm trên Uno Q
...
# Clone llamafarm (hoặc scp project của bạn)
...
# Copy sang Uno Q
scp target/aarch64-unknown-linux-gnu/release/llamafarm arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/llamafarm /usr/local/bin/"
```
...
# Cấu hình nhanh
llamafarm onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter

# Hoặc tạo config thủ công
mkdir -p ~/.llamafarm/workspace
nano ~/.llamafarm/config.toml
```
...
## Phase 4: Chạy LlamaFarm Daemon

```bash
ssh arduino@<UNO_Q_IP>

# Chạy daemon (Telegram polling hoạt động qua WiFi)
llamafarm daemon --host 127.0.0.1 --port 3000
```

**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — LlamaFarm phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (LlamaFarm xử lý tự động)

LlamaFarm bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo llamafarm):
```bash
llamafarm peripheral setup-uno-q --host 192.168.0.48
```

**Từ Uno Q** (đã SSH vào):
```bash
llamafarm peripheral setup-uno-q
```

Lệnh này copy Bridge app vào `~/ArduinoApps/llamafarm-uno-q-bridge` và khởi động nó.
...
Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, LlamaFarm dùng `gpio_write` qua Bridge.
...
| 5 | `git clone https://github.com/theonlyhennygod/llamafarm.git && cd llamafarm` |
| 6 | `cargo build --release --no-default-features` |
| 7 | `llamafarm onboard --api-key KEY --provider openrouter` |
| 8 | Chỉnh sửa `~/.llamafarm/config.toml` (thêm Telegram bot_token) |
| 9 | `llamafarm daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: llamafarm"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/llamafarm` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`llamafarm peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. LlamaFarm dùng endpoint v4 chính xác.
