...
|---|---|---|
| Foreground runtime | `llamafarm daemon` | gỡ lỗi cục bộ, phiên ngắn |
| Foreground gateway only | `llamafarm gateway` | kiểm thử webhook endpoint |
| User service | `llamafarm service install && llamafarm service start` | runtime được quản lý liên tục bởi operator |
...
|---|---|---|
| Tính hợp lệ của config | `llamafarm doctor` | không có lỗi nghiêm trọng |
| Kết nối channel | `llamafarm channel doctor` | các channel đã cấu hình đều khoẻ mạnh |
| Tóm tắt runtime | `llamafarm status` | provider/model/channels như mong đợi |
| Heartbeat/trạng thái daemon | `~/.llamafarm/daemon_state.json` | file được cập nhật định kỳ |
...
1. Nếu các channel vẫn thất bại, kiểm tra allowlist và thông tin xác thực trong `~/.llamafarm/config.toml`.
...
1. sao lưu `~/.llamafarm/config.toml`
2. chỉ áp dụng một thay đổi logic tại một thời điểm
3. chạy `llamafarm doctor`
