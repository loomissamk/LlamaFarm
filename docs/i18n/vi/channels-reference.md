...
Tài liệu này là nguồn tham khảo chính thức về cấu hình channel trong LlamaFarm.
...
6. **Trạng thái runtime cũ**: config đã thay đổi nhưng `llamafarm daemon` chưa được khởi động lại.

---

## 1. Namespace cấu hình

Tất cả cài đặt channel nằm trong `channels_config` trong `~/.llamafarm/config.toml`.
...
Khi chạy `llamafarm channel start` (hoặc chế độ daemon), Telegram và Discord hỗ trợ chuyển đổi runtime theo phạm vi người gửi:
...
- Xem trước bộ nhớ cache model từ `llamafarm models refresh --provider <ID>`.
- Đây là lệnh chat runtime, không phải lệnh con CLI.

## Giao thức marker hình ảnh đầu vào

LlamaFarm hỗ trợ đầu vào multimodal qua các marker nội tuyến trong tin nhắn:
...
Nếu `[channels_config.matrix]`, `[channels_config.lark]`, hoặc `[channels_config.feishu]` có mặt nhưng binary được build mà không có feature tương ứng, các lệnh `llamafarm channel list`, `llamafarm channel doctor`, và `llamafarm channel start` sẽ ghi log rằng channel đó bị bỏ qua có chủ ý trong bản build này.
...
access_token = "syt_..."
user_id = "@llamafarm:matrix.example.com"   # tùy chọn, khuyến nghị cho E2EE
...
LlamaFarm hỗ trợ hai backend WhatsApp:
...
5. Khởi động lại `llamafarm daemon` sau khi thay đổi config.
...
```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Webhook|Channel" /tmp/llamafarm.log
```
