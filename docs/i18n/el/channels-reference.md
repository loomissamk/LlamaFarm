...
Αυτός ο οδηγός περιγράφει τη διαδικασία διαμόρφωσης των καναλιών επικοινωνίας (Telegram, Discord, κ.λπ.) στο LlamaFarm.
...
Οι ρυθμίσεις αποθηκεύονται στο αρχείο `~/.llamafarm/config.toml`, στην ενότητα `[channels_config]`.
...
Το LlamaFarm υποστηρίζει την ανάγνωση εικόνων μέσω της σύνταξης:
`[IMAGE:<source>]`
...
5. **Επανεκκίνηση**: Κάθε αλλαγή στο `config.toml` απαιτεί επανεκκίνηση του `llamafarm daemon`.

### Διαγνωστικά Μηνύματα (Logs)

Εκτελέστε το LlamaFarm με αυξημένο επίπεδο καταγραφής:
```bash
