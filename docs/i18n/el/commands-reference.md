# Αναφορά Εντολών LlamaFarm (CLI Reference)
...
| `daemon` | Εκτέλεση του LlamaFarm ως διεργασία παρασκηνίου (Background Process). |
...
- `llamafarm onboard --interactive`: Διαδραστική καθοδήγηση για τη ρύθμιση του συστήματος.
- `llamafarm onboard --channels-only`: Εστιασμένη διαμόρφωση αποκλειστικά για τα κανάλια επικοινωνίας.

### 2. `agent` (Διαδραστική Λειτουργία)

- `llamafarm agent`: Έναρξη τυπικής συνομιλίας.
- `llamafarm agent -m "<μήνυμα>"`: Άμεση αποστολή εντολής/μηνύματος στον πράκτορα.
...
- `llamafarm cron add "0 9 * * *" "echo Daily Setup"`: Εκτέλεση καθημερινά στις 09:00.
- `llamafarm cron once "1h" "llamafarm status"`: Προγραμματισμός εκτέλεσης μετά από μία ώρα.

### 4. `doctor` (Διάγνωση Συστήματος)

Χρησιμοποιήστε την εντολή `llamafarm doctor` για την επαλήθευση της ορθής λειτουργίας των εξαρτήσεων, της πρόσβασης στο διαδίκτυο και της εγκυρότητας του αρχείου ρυθμίσεων.

### 5. `skills` (Επεκτασιμότητα)

- `llamafarm skills list`: Προβολή εγκατεστημένων δεξιοτήτων.
- `llamafarm skills install <source>`: Εγκατάσταση νέας δεξιότητας από εξωτερική πηγή.

> [!NOTE]
> Το LlamaFarm εφαρμόζει αυτόματη ανάλυση κώδικα (security scanning) σε κάθε νέα δεξιότητα πριν την ενεργοποίησή της για την αποφυγή εκτέλεσης κακόβουλου λογισμικού.
