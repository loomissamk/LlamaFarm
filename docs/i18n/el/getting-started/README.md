...
Οδηγίες για την αρχική εγκατάσταση και την εξοικείωση με το οικοσύστημα του LlamaFarm.
...
|:---|:---|
| Διαθέτω API Key και επιθυμώ άμεση ρύθμιση | `llamafarm onboard --api-key sk-... --provider openrouter` |
| Επιθυμώ διαδραστική καθοδήγηση (Step-by-step) | `llamafarm onboard --interactive` |
| Επιθυμώ μόνο τη ρύθμιση καναλιών επικοινωνίας | `llamafarm onboard --channels-only` |
| Επιθυμώ πλήρη αντικατάσταση υπαρχουσών ρυθμίσεων | `llamafarm onboard --force` |
...
- **Έλεγχος Υγείας**: Μετά την εγκατάσταση, εκτελέστε τις εντολές `llamafarm status` και `llamafarm doctor` για την επαλήθευση της σωστής λειτουργίας.
