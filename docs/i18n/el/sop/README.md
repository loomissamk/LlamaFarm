...
- Το CLI `llamafarm sop` διαχειρίζεται αυτή τη στιγμή μόνο ορισμούς: `list`, `validate`, `show`.
...
2. Δημιουργήστε έναν κατάλογο SOP, για παράδειγμα:
   ...
   ```text
   ~/.llamafarm/workspace/sops/deploy-prod/SOP.toml
   ~/.llamafarm/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Επαλήθευση και επιθεώρηση ορισμών:
   ...
   llamafarm sop show deploy-prod
   ```
