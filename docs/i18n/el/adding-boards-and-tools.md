# Προσθήκη Πλακετών και Εργαλείων — Οδηγός Υλικού LlamaFarm

Αυτός ο οδηγός εξηγεί πώς να προσθέσετε νέες πλακέτες υλικού και προσαρμοσμένα εργαλεία στο LlamaFarm.

## Γρήγορη Εκκίνηση: Προσθήκη Πλακέτας μέσω CLI

```bash
# Προσθήκη πλακέτας (ενημερώνει το ~/.llamafarm/config.toml)

llamafarm peripheral add nucleo-f401re /dev/ttyACM0
llamafarm peripheral add arduino-uno /dev/cu.usbmodem12345
llamafarm peripheral add rpi-gpio native   # για Raspberry Pi GPIO (Linux)
...
Με τη δυνατότητα rag-pdf, το LlamaFarm μπορεί να ευρετηριάσει αρχεία PDF:
...
2. Προσθήκη στις ρυθμίσεις — llamafarm peripheral add my-board /dev/ttyUSB0
...
| Εντολή | Περιγραφή |
|---------|-------------|
| llamafarm peripheral list | Λίστα ρυθμισμένων πλακετών |
| llamafarm peripheral add <board> <path> | Προσθήκη πλακέτας (εγγραφή στο config) |
| llamafarm peripheral flash | Μεταφόρτωση (flash) υλικολογισμικού Arduino |
| llamafarm peripheral flash-nucleo | Μεταφόρτωση (flash) υλικολογισμικού Nucleo |
| llamafarm hardware discover | Λίστα συσκευών USB |
| llamafarm hardware info | Πληροφορίες chip μέσω probe-rs |
