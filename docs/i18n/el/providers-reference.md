# Αναφορά Παρόχων LlamaFarm (Providers Reference)
...
Το runtime του LlamaFarm αναζητά διαπιστευτήρια με την εξής σειρά προτεραιότητας:
...
3. **Γενικές μεταβλητές**: Εφεδρικές μεταβλητές όπως οι `LLAMAFARM_API_KEY` ή `API_KEY`.
...
- Τα μοντέλα συλλογιστικής (thinking models) υποστηρίζονται εγγενώς· το LlamaFarm φιλτράρει αυτόματα τα εσωτερικά metadata της συλλογιστικής.
...
- **Adaptive Context**: Το `provider.ollama_num_ctx` είναι ακριβής χειροκίνητη
  παράκαμψη. Όταν δεν ορίζεται, το Auto χρησιμοποιεί το εγγενές μέγιστο του
  επιλεγμένου μοντέλου. Το `OLLAMA_NUM_CTX` γίνεται γρήγορη βάση μόνο με
  `LLAMAFARM_ADAPTIVE_CONTEXT=true` και δεν περιορίζει το κανονικό Auto, έως το μικρότερο από το εγγενές
...
Κλήση μέσω CLI: `llamafarm agent --model hint:fast --message "..."`.
