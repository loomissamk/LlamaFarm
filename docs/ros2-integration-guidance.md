...
It is intentionally architecture-focused and keeps LlamaFarm core boundaries stable.

## Recommendation

Use the plugin/adapter route first.

- Keep robotics transport in an integration crate or module that bridges ROS topics/services/actions to LlamaFarm tools/channels/runtime adapters.
- Keep high-frequency control loops in ROS-native execution contexts.
- Use LlamaFarm for planning, orchestration, policy, and guarded action dispatch.
...
- Security posture: approval, policy, and gating remain centralized in existing LlamaFarm paths.
...
- LlamaFarm emits intent-level commands and receives summarized state.
...
2. Bridge performs local reduction/windowing and forwards compact summaries to LlamaFarm.
3. LlamaFarm decides intent/tool calls under existing policy and approval constraints.
4. Bridge translates approved intents into ROS commands with bounded command-rate limits.
5. Telemetry and fault states flow back into LlamaFarm for reasoning and auditability.

## Escalation Criteria For Core Integration

Consider deeper LlamaFarm runtime integration only when all are true:
