# LlamaFarm Security Improvement Roadmap
...
LlamaFarm already has **excellent application-layer security**:
...
## Comparison: LlamaFarm vs PicoClaw vs Production Grade

| Feature | PicoClaw | LlamaFarm Now | LlamaFarm + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`llamafarm audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/llamafarm/audit.log"
sign_events = true
max_size_mb = 100
...
llamafarm security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
llamafarm audit --user @alice --since 24h
llamafarm audit --risk high --violations-only
llamafarm audit --verify-signatures

# Sandbox test
llamafarm sandbox --test
# → Testing isolation...
...
**LlamaFarm is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, LlamaFarm becomes:
...
**Value**: Transforms LlamaFarm from "safe for testing" to "safe for production"
