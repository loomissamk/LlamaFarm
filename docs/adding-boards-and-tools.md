# Adding Boards and Tools — LlamaFarm Hardware Guide

This guide explains how to add new hardware boards and custom tools to LlamaFarm.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.llamafarm/config.toml)
llamafarm peripheral add nucleo-f401re /dev/ttyACM0
llamafarm peripheral add arduino-uno /dev/cu.usbmodem12345
llamafarm peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, LlamaFarm can index PDF files:
...
2. **Add to config** — `llamafarm peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `llamafarm peripheral list` | List configured boards |
| `llamafarm peripheral add <board> <path>` | Add board (writes config) |
| `llamafarm peripheral flash` | Flash Arduino firmware |
| `llamafarm peripheral flash-nucleo` | Flash Nucleo firmware |
| `llamafarm hardware discover` | List USB devices |
| `llamafarm hardware info` | Chip info via probe-rs |
