# Hardware Peripherals Design — LlamaFarm

LlamaFarm enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** LlamaFarm acts as a hardware-aware AI agent that:
...
**Mental model:** LlamaFarm = brain that understands hardware. Peripherals = arms and legs it controls.
...
LlamaFarm runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.
...
2. LlamaFarm fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
LlamaFarm runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.
...
2. LlamaFarm identifies connected hardware (VID/PID, architecture)
...
2. LlamaFarm writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. LlamaFarm auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| LlamaFarm runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs LlamaFarm; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

LlamaFarm on Pi; GPIO via rppal or sysfs. No separate firmware.
...
1. **Startup:** LlamaFarm loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between LlamaFarm and peripherals:
...
- **llamafarm-firmware** or **llamafarm-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; LlamaFarm connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`llamafarm peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `llamafarm hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `llamafarm hardware introspect <path>`: memory map, peripheral list
...
- [x] LlamaFarm on Raspberry Pi (native GPIO via rppal)
...
- [x] `llamafarm-esp32` firmware crate (`firmware/llamafarm-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] LlamaFarm *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/llamafarm-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full LlamaFarm *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). LlamaFarm runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. LlamaFarm gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: LlamaFarm from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey LlamaFarm, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
