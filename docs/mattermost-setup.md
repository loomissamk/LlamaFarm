...
LlamaFarm supports native integration with Mattermost via its REST API v4. This integration is ideal for self-hosted, private, or air-gapped environments where sovereign communication is a requirement.
...
2.  **Bot Account**:
    ...
    - Set a username (e.g., `llamafarm-bot`).
...
| `mention_only` | (Optional) When `true`, only messages that explicitly mention the bot username (for example `@llamafarm-bot`) are processed. Default: `false`. |

## Threaded Conversations

LlamaFarm supports Mattermost threads in both modes:
- If a user sends a message in an existing thread, LlamaFarm always replies within that same thread.
...
When `mention_only = true`, LlamaFarm applies an extra filter after `allowed_users` authorization:
