...
LlamaFarm supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
- Auto mode: when your `custom:` endpoint resolves to `api.openai.com`, LlamaFarm will try WebSocket mode first (`wss://.../responses`) and automatically fall back to HTTP if the websocket handshake or stream fails.
- Manual override:
  - `LLAMAFARM_RESPONSES_WEBSOCKET=1` forces websocket-first mode for any `custom:` endpoint.
  - `LLAMAFARM_RESPONSES_WEBSOCKET=0` disables websocket mode and uses HTTP only.
...
# or: export LLAMAFARM_API_KEY="your-api-key"
llamafarm agent
```

## Hunyuan (Tencent)

LlamaFarm includes a first-class provider for [Tencent Hunyuan](https://hunyuan.tencent.com/):
...
LlamaFarm includes a first-class local provider for `llama-server`:
...
```bash
llamafarm models refresh --provider llamacpp
llamafarm agent -m "hello"
```

You do not need to export `LLAMAFARM_API_KEY=dummy` for this flow.

## SGLang Server

LlamaFarm includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
llamafarm models refresh --provider sglang
llamafarm agent -m "hello"
```
...
LlamaFarm includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
llamafarm models refresh --provider vllm
llamafarm agent -m "hello"
```
...
# Single message test
llamafarm agent -m "test message"
```
