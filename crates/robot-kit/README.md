...
A complete toolkit for building AI-powered robots with LlamaFarm. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build LlamaFarm with robot tools

```bash
# Clone and build
git clone https://github.com/llamafarm-labs/llamafarm
cd llamafarm
cargo build -p llamafarm-robot-kit --release
```
...
# Test in mock mode
./target/release/llamafarm agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/llamafarm agent -m "Move forward 1 meter"
```
...
```rust
use llamafarm_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `llamafarm`, add a thin adapter that maps this
...
mkdir llamafarm-robot-kit
cp -r target/release/llamafarm llamafarm-robot-kit/
cp -r examples/robot_kit llamafarm-robot-kit/
cp -r ~/.llamafarm llamafarm-robot-kit/dot-llamafarm

# Include models
mkdir -p llamafarm-robot-kit/models
cp ~/.llamafarm/models/ggml-base.bin llamafarm-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf llamafarm-robot-kit.tar.gz llamafarm-robot-kit/

# Copy to USB
cp llamafarm-robot-kit.tar.gz /media/usb/TarBalls/
```
