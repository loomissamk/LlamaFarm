...
Complete guide to setting up a LlamaFarm-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.llamafarm/models
cp models/ggml-base.bin ~/.llamafarm/models/
```
...
mkdir -p ~/.llamafarm/models/piper
cd ~/.llamafarm/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.llamafarm/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build LlamaFarm Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/llamafarm
cd llamafarm

# Build robot kit
cargo build --release -p llamafarm-robot-kit

# Build main llamafarm (optional, if using as agent)
cargo build --release
```
...
```toml
# ~/.llamafarm/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.llamafarm/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/llamafarm_sensors.fifo"
...
### Start LlamaFarm Agent

```bash
# Configure LlamaFarm to use robot tools
cat > ~/.llamafarm/config.toml << 'EOF'
...
# Start sensor loop
if [ ! -p /tmp/llamafarm_sensors.fifo ]; then
    mkfifo /tmp/llamafarm_sensors.fifo
fi
...
# Start llamafarm
cd ~/llamafarm
./target/release/llamafarm daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/llamafarm-robot.service
sudo tee /etc/systemd/system/llamafarm-robot.service << 'EOF'
[Unit]
Description=LlamaFarm Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/llamafarm
...
sudo systemctl enable llamafarm-robot
sudo systemctl start llamafarm-robot

# Check status
sudo systemctl status llamafarm-robot
journalctl -u llamafarm-robot -f  # View logs
```
