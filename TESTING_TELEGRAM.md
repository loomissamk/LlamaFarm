...
This guide covers testing the Telegram channel integration for LlamaFarm.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug llamafarm channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: llamafarm onboard --channels-only
```
...
# 3. Monitor logs
RUST_LOG=info llamafarm daemon
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time llamafarm channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep llamafarm`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/llamafarm` |
...
```bash
# Maximum logging
RUST_LOG=trace llamafarm channel start
...
# Validate config
cat ~/.llamafarm/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [LlamaFarm Main README](README.md)
- [Contributing Guide](CONTRIBUTING.md)
- [Issue Tracker](https://github.com/theonlyhennygod/llamafarm/issues)
