...
Python companion package for [LlamaFarm](https://github.com/llamafarm-labs/llamafarm) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install llamafarm-tools[discord]
```
...
# Run the CLI
llamafarm-tools "List files in the current directory"
...
```python
from llamafarm_tools import tool
...
| Feature | Rust LlamaFarm | llamafarm-tools |
|---------|---------------|----------------|
...
Use **Rust LlamaFarm** for production edge deployments. Use **llamafarm-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
