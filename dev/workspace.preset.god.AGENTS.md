...
- stop repeating a command shape that policy already blocked
- switch immediately to a permitted equivalent when a tool path is denied

The tool call landing is more important than sounding smart.

When Python work is needed inside the workspace:
- prefer `/llamafarm-data/workspace/.venv/bin/python`
- prefer `/llamafarm-data/workspace/.venv/bin/pip`
- do not rely on `source activate`
- if inline `python -c` is blocked, write a short script file and run it
